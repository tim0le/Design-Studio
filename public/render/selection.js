/*
 * Furgoson Studio — client selection layer.
 *
 * Ported verbatim from the server's SELECTION_SCRIPT (lib/marp.js). In the
 * original the script lived inside a JS template literal, so regex escape
 * sequences were DOUBLE-escaped (e.g. `\\(` `\\s`). Here, as a real .js file,
 * they are restored to SINGLE escapes so the regex literal is correct.
 *
 * This module exposes the exact `<style>...</style><script>...</script>` block
 * the server used to append before </body>, as `window.FGS_SELECTION_HTML`, so
 * a renderer can embed it directly into an iframe document. The behavior — and
 * crucially every postMessage payload (`element-selected`, `text-edited`,
 * `drag-position`, `element-delete`) — is byte-identical to the original so the
 * parent window's handlers keep working unchanged.
 *
 * Touch consideration (per the PWA plan): the drag uses pointer events, which
 * already work for touch, but `.fgs-selected` gets `touch-action: none` so
 * dragging a selected element on a touchscreen doesn't fight page scroll.
 *
 * Plain browser JS, no modules.
 */
(function () {
  var FGS_SELECTION_HTML = [
'<style>',
...
'.fgs-selected { outline: 2px solid #e55a3e !important; outline-offset:3px; background:rgba(229,90,62,0.06) !important; cursor:move !important; touch-action: none !important; }',
...
'       sees them. Detect a clear horizontal flick here and forward it as an',
'       fgs-swipe message; the parent maps dir<0 -> next, dir>0 -> prev. We skip',
'       it while dragging or inline-editing an element. */',
'    var _sw = null;',
...
'<\/script>'
  ...
  if (typeof window !== 'undefined') {
    window.FGS_SELECTION_HTML = FGS_SELECTION_HTML;
  }
  // Also support CommonJS so build tooling / Node tests can read the string.
  if (typeof module !== 'undefined' && module.exports) {
    module.exports = { FGS_SELECTION_HTML: FGS_SELECTION_HTML };
  }
})();
