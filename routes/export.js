...
const DECK_FORMATS = ['pdf', 'pptx'];
const SLIDE_FORMATS = ['pdf', 'pptx', 'png', 'jpg', 'jpeg', 'html'];

router.post('/', (req, res) => {
  const { deckId, format } = req.body;
  if (!deckId || !format) return res.status(400).json({ error: 'deckId and format required' });
  if (!DECK_FORMATS.includes(format)) return res.status(400).json({ error: 'format must be pdf or pptx' });
